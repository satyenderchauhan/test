...
class Owners extends CI_Controller {
	...
	public function onwerList()
	{
        ...
        $ptable = 'owners as o JOIN user_accounts as ua on ua.id = o.user_account_id ';

        $where = ' where ua.user_type = 1 ';
        $order = '';
        if (isset($in['search'])) {
            if ($in['search']['value'] != '') {
                $search = '';
    ...
    public function changeStatus()
    {
        ...
        die(json_encode(['status'=>'200','msg'=> 'Owner '.$data['status'].'d Successfuly']));
    }
}
